...
# --- 3. LIVE BINANCE DATA FETCHING ENGINE ---
def fetch_binance_live_ticker():
    try:
        # Direct Binance REST API ticker endpoint for instant live price
        ...
        # Calculate micro-price and imbalance proxies from top of book
...
# Always fetch fresh live price on every rerun cycle
...
def load_and_process_data():
    ...
    try:
        ...
        # Trend Indicators
        df['ema_100'] = df['mid_price'].ewm(span=30, adjust=False).mean()
    ...
    if enable_paper_trader:
        ...
        if not st.session_state.positions:
            ...
            if is_bullish_regime and ml_prob_long >= min_confidence and current_mlofi_z > z_threshold:
                ...
                st.success(f"🚀 ML Executed LONG @ ${executed_entry:,.2f} | Prob: {ml_prob_long:.2%}")
                ...
                st.success(f"🔻 ML Executed SHORT @ ${executed_entry:,.2f} | Prob: {ml_prob_short:.2%}")
        ...
        if st.session_state.positions:
            ...
            p_col2.metric("Entry", f"${entry_p:,.2f}")
            p_col3.metric("Gross PnL", f"${live_gross:,.2f}")
            p_col4.metric("Net PnL", f"${live_net:,.2f}", delta=f"-${live_friction:.2f} Fees/Slippage")
...
# --- 8. AUTO-RERUN EVERY 2 SECONDS FOR REAL-TIME GRAPH TICKING ---
time.sleep(2)
st.rerun()
